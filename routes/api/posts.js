...
router.post(
  ...
  [
    auth,
    [
      check('text', 'se requiere escribir algun texto')
        .not()
        .isEmpty()
    ]
  ],
...
router.put('/like/:id', auth, async (req, res) => {
  try {
    ...
    //checkear si el post ya tiene like
    if (
      post.likes.filter(like => like.user.toString() === req.user.id).length > 0
    ) {
      return res.status(400).json({
        msg: 'este post ya tiene tu like'
      });
    }
...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        msg: 'este post todavia no tiene likes'
      });
    }
